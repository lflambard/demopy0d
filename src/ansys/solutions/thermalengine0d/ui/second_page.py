...
def layout(step: SecondStep):
    ...
    return html.Div(
        [
            ...
            dcc.Markdown("""###### Plot your Results.""", className="display-3"),
            html.Hr(className="my-2"),
            html.Br(),
            html.Div(["Plot in X: ", dcc.Input(id="first-arg", value=step.first_arg, type="number")]),
            html.Div(["Plot in Y: ", dcc.Input(id="second-arg", value=step.second_arg, type="number")]),
            html.Button("Plots", id="plots", n_clicks=0),
...
@callback(
    ...
    Input("plots", "n_clicks"),
    State("first-arg", "value"),
    State("second-arg", "value"),
    State("url", "pathname"),
    prevent_initial_call=True,
)
def plots(n_clicks, first_arg, second_arg, pathname):
    ...
    step.plots()
    return f"Done within: {step.result}s"
