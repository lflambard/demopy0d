...
from ansys.solutions.thermalengine0d.model.Library_Mechanics_class import Shaft_I, Propeller_Tf, Hydro1D_I
...
result_simu = np.zeros((int(LastVal+1 * step_simu / sample_time), 18))
...
for i in range(1, int(LastVal+1)):
    ...
    if eval(config.get('Hosepipe_param','length')) > 0:
        F_HosePipe = interpolv(eval(config.get('Hosepipe_param','x_f_hosepipe')), eval(config.get('Hosepipe_param','y_f_hosepipe')), Hydro.X)
    else:
        F_HosePipe = 0
    ...
    if incr_save > incr_save_old:
        ...
        result_simu[incr_save, 9] = Hydro.Acc_X
        result_simu[incr_save, 10] = Hydro.Fmt.u / 1852 * 3600
        result_simu[incr_save, 11] = Electronic_section.V_ord
        result_simu[incr_save, 12] = Hydro.X
